...
WithDeadline::WithDeadline(time deadlineP, std::shared_ptr<Context> parentP)
    : WithCancel(std::move(parentP)), deadlineVar(deadlineP), waitingThread{ [this]() {
...
WithDeadline::~WithDeadline() { cancel(); }

std::optional<time> WithDeadline::deadline() const { return deadlineVar; }
